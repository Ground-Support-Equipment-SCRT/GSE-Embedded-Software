...
void Rfm69::send(const uint8_t *data, uint8_t len) {
    setMode(REG_OPMODE_STDBY);
    ...
    setMode(0x0C); // TX mode
    while (!(readReg(REG_IRQFLAGS2) & 0x08)); // Wait for PacketSent
    setMode(REG_OPMODE_RX); // Return to RX mode
}
